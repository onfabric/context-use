...
class TestE2EErrors:
    def test_bad_zip(self, ctx: ContextUse, tmp_path: Path):
        bad = tmp_path / "bad.zip"
        bad.write_bytes(b"not a zip file")

        with pytest.raises(ArchiveProcessingError):
            ctx.process_archive(Provider.CHATGPT, str(bad))

    def test_unsupported_provider(self, ctx: ContextUse, tmp_path: Path):
    ...
    def test_empty_archive(self, ctx: ContextUse, tmp_path: Path):
        ...
        result = ctx.process_archive(Provider.CHATGPT, str(empty_zip))
        assert result.tasks_completed == 0
        assert result.threads_created == 0

    def test_corrupt_json(self, ctx: ContextUse, tmp_path: Path):
        ...
        result = ctx.process_archive(Provider.CHATGPT, str(bad_json_zip))
