...
class ThreadEmbeddingBatchManager(BaseBatchManager):
    """Embeds thread content into vec_threads.
    ...
    async def _transition(self, current_state: State) -> State | None:
        match current_state:
            case CreatedState():
                logger.info("[%s] Starting thread embedding", self.batch.id)
                return await self._submit_embeddings()

            case ThreadEmbedPendingState() as state:
                logger.info("[%s] Polling thread embedding status", self.batch.id)
                return await self._check_embedding_results(state)

            case ThreadEmbedCompleteState():
                logger.info("[%s] Thread embedding complete", self.batch.id)
                return CompleteState()

            case _:
                raise ValueError(
                    f"Invalid state for thread_embedding batch: {current_state}"
                )
    ...
    async def _submit_embeddings(self) -> State:
        ...
        job_key, _ = await submit_thread_embeddings(
            embeddable, self.batch.id, self.ctx.llm_client
        )
        return ThreadEmbedPendingState(job_key=job_key)

    async def _check_embedding_results(self, state: ThreadEmbedPendingState) -> State:
