...
from context_use.providers.instagram.connections.schema_followers import (
    Model as FollowerItem,
)
from context_use.providers.instagram.connections.schema_following import (
    Model as FollowingManifest,
)
...
def _parse_follower_item(item: FollowerItem) -> InstagramConnectionRecord:
    ...
    def extract_file(
        ...
        try:
            for raw in ijson.items(stream, "item"):
                item = FollowerItem.model_validate(fix_strings_recursive(raw))
                yield _parse_follower_item(item)
        finally:
            stream.close()
...
class InstagramFollowingPipe(_InstagramConnectionPipe):
    ...
    def extract_file(
        ...
        manifest = FollowingManifest.model_validate(data)
