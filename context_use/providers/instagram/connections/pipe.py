...
from context_use.providers.instagram.connections.schemas_followers import (
    Model as FollowerItem,
)
from context_use.providers.instagram.connections.schemas_following import (
    ...
    def extract_file(
        ...
        try:
            for raw in ijson.items(stream, "item"):
                item = FollowerItem.model_validate(raw)
                sld = item.string_list_data[0]
                username = (
                    sld.href.rstrip("/").split("/")[-1]
                    if sld.href
                    else (sld.value or "")
                )
...
class InstagramFollowingPipe(_InstagramConnectionPipe):
    ...
    def extract_file(
        ...
        manifest = FollowingManifest.model_validate_json(raw)
