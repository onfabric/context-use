...
class EmbedCommand(ApiCommand):
    ...
    description = (
        "Generate embeddings for thread content and store them in the "
        "vector table. Processes all unprocessed threads by default. "
    ...
    async def run(
        ...
        out.header("Embedding thread content")
        out.info("Processes all threads that have not been embedded yet.")
        if since:
